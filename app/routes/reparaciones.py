...
def nueva_reparacion():
    if request.method == 'POST':
        nueva_rep = Reparacion(
            ...
            costo=request.form['costo'] if request.form['costo'] else None,  # Permitir valores vacíos
            fecha_entrega=None,
...
@bp.route('/reparacion/<int:reparacion_id>')  # Faltaba el parámetro <int:reparacion_id>
@login_required
...
@bp.route('/reparacion/editar/<int:reparacion_id>', methods=['GET', 'POST'])  # Faltaba el parámetro
@login_required
def editar_reparacion(reparacion_id):
    ...
    if request.method == 'POST':
        ...
        reparacion.costo = request.form['costo'] if request.form['costo'] else None
...
def estadisticas_reparaciones():
    ...
    total_reparaciones = Reparacion.query.count()
    reparaciones_completadas = Reparacion.query.filter_by(estado='completada').count()
    reparaciones_pendientes = Reparacion.query.filter_by(estado='pendiente').count()
    reparaciones_en_proceso = Reparacion.query.filter_by(estado='en_proceso').count()
    
    estadisticas = {
        'total': total_reparaciones,
        'completadas': reparaciones_completadas,
        'pendientes': reparaciones_pendientes,
        'en_proceso': reparaciones_en_proceso
    }
